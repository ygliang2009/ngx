...
/*
 * 接收到事件的初始化连接函数，当中会调用ngx_http_init_request构造请求选项
 */
void
...
ngx_http_init_request(ngx_event_t *rev)
{
	...
	/*
	 * 建立ngx_http_request_t到ngx_connection_t的双向映射关系
	 */
    c->data = r;
	...
	/*
	 * 将server项信息保存到ngx_http_request_t结构中
	 */
    r->main_conf = cscf->ctx->main_conf;
    r->srv_conf = cscf->ctx->srv_conf;
    r->loc_conf = cscf->ctx->loc_conf;
	/*
	 * 修改接收事件回调句柄为ngx_http_process_request_line
	 */
    ...
    if (c->buffer == NULL) {
		/*
		 * 连接为http头建立了内存池，内存池大小可以由client_header_buffer_size决定
		 */
...
#endif
	/*
	 *  调用412行修改的回调函数 ngx_http_process_request_line;
	 */
    rev->handler(rev);
}
...
ngx_http_process_request_line(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
		/*
		 * 第二次执行以NGX_AGAIN为标志，执行ngx_http_read_request_header方法
		 */
        if (rc == NGX_AGAIN) {
			/*
			 * 把TCP流复制到request的header_in缓冲区
			 */
        ...
        if (rc == NGX_OK) {
			...
			/*
			 * 如果版本小于1.0，寻找虚拟主机配置存入r中，并调用process_request
			 */
...
/*
 * 接收完完整的HTTP请求头部后，已经有足够的必要信息开始在业务上处理HTTP请求了
 */
static void
...
#endif
	/*
	 * 重新设置连接读写回调函数，如果一次没有处理完HTTP请求，则
	 * 下一次处理请求调用ngx_http_request_handler，epoll事件注册
	 * 的便是connection的读写事件handler
	 */
    c->read->handler = ngx_http_request_handler;
    c->write->handler = ngx_http_request_handler;
	/*
	 * 设置request回调函数，由connection的读写事件调用
	 */
    r->read_event_handler = ngx_http_block_reading;
	/*
	 * 处理请求
